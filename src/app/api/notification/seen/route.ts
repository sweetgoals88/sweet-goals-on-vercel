...
export async function GET(request: NextRequest) {
    try {
        ...
        const seenNotificationsQuery = query(
            FirebaseConfiguration.NOTIFICATIONS,
            where("userId", "==", userId),
            where("seen_at", "!=", null)
        );
        const notificationsSnapshot = await getDocsFromServer(seenNotificationsQuery);
        ...
        return makeErrorResponse("Couldn't fetch seen notifications", 500, error);
    }
}